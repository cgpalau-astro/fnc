...
def test_monte_carlo_markov_chain():
    ...
    norm_mean = np.mean(sample) / MEAN_MC
    norm_std = np.std(sample) / STD_MC

    assert np.isclose(norm_mean, 1.0, atol=1.0E-1)
    assert np.isclose(norm_std, 1.0, atol=1.0E-1)
...
def test_sigma_error():
    ...
    norm = scipy.stats.norm(loc=mu, scale=scale)

    x = np.linspace(x_inf, x_sup, 1_000)
    y = norm.pdf(x)

    error = fnc.stats.sigma_error(x, y, mu, x_inf, x_sup, x0_inf=mu-eps, x0_sup=mu+eps, verbose=False)

    assert np.all( np.isclose(error/scale, 1.0, rtol=0.3E-4) )
    assert np.isclose( scipy.integrate.quad(norm.pdf, mu-scale, mu+scale)[0]/fnc.stats.norm.sigma_level(1), 1.0 )
